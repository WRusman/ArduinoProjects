...
int calcYear,calcMonth,calcDay,calcHour,calcMinute,calcSecond,toDateHours,toDateMinutes,toDateSeconds,countDownHours,countDownMinutes,countDownSeconds,diffUTC,buttonState,lastButtonState = LOW ;

char countDownDate[16];
String talkEnd="00:00", clockState="Pause", clockMode, htmlClockColor;
RGB clockColor;
...
void setup() {
  ...
  server.on("/readData", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    if (countDownHours == -1){
    timeRemaining = "-"+String((countDownMinutes)/10 %10)+String((countDownMinutes) %10)+":"+String((countDownSeconds/10) %10) +String((countDownSeconds) %10);
    }
    else
    {
    timeRemaining = String((countDownHours) %10)+":"+String((countDownMinutes)/10 %10)+String((countDownMinutes) %10)+":"+String((countDownSeconds/10) %10) +String((countDownSeconds) %10);
    }
    request->send(200, "application/json","{\"localtime\":\""+localTime+"\",\"endtime\":\""+talkEnd+"\",\"timeremaining\":\""+timeRemaining+"\",\"clockstate\":\""+clockState+"\",\"clockcolor\":\""+htmlClockColor+"\"}");
  });
...
void loop() {
  ...
  timeClient.update();  
    ...
    if (clockMode == "TZ")
    {
      ...
      countDownMinutes = toDateMinutes - (toDateHours  * 60); 
      countDownSeconds = toDateSeconds - (countDownMinutes * 60) - ((toDateHours * 60) *60); 
    ...
    else
    {
      ...
      countDownMinutes = toDateMinutes - (toDateHours * 60); 
      countDownSeconds = toDateSeconds - (countDownMinutes * 60) - ((toDateHours * 60) *60); 
      if ((countDownSeconds < 0) or (countDownMinutes < 0)){
        countDownHours = -1; 
        countDownSeconds = -(toDateSeconds - (countDownMinutes * 60) - ((toDateHours * 60) *60)); 
        countDownMinutes = -(toDateMinutes - (toDateHours * 60)); 
      }
    ...
    if (clockState == "On")
    {
      Serial.println(String(countDownHours)+":"+String(countDownMinutes)+":"+String((countDownSeconds/10) %10) +String((countDownSeconds) %10));
      if (toDateSeconds <= 0 ){
        clockColor=red;
        htmlClockColor="red";
      }
      else if ((toDateMinutes < 5 ) and (toDateMinutes >= 0 )) {
        clockColor=orange;      
        htmlClockColor="orange";              
      }
      else if ((toDateMinutes < 10 ) and (toDateMinutes >= 5 )){
        clockColor=yellow;
        htmlClockColor="yellow";
      }
      else{
        clockColor=white;
        htmlClockColor="white";
      }
      showTime(countDownHours, countDownMinutes, countDownSeconds, clockColor);
    }
    delay(1000);
}
