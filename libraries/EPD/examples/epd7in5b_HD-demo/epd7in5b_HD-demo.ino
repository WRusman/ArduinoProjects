...
void setup() {
 DEBUG("7.5inch HD e-Paper B demo\r\n");
 DEV_Module_Init();
  ...
  DEV_Delay_ms(300);
    ...
    //Draw black picture
    DEBUG("IMAGE_BW\n");
    Paint_NewImage(IMAGE_BW, EPD_7IN5B_HD_WIDTH, EPD_7IN5B_HD_HEIGHT, IMAGE_ROTATE_0, IMAGE_COLOR_POSITIVE);
    Paint_Clear(WHITE);
    //3.Read BMP pictures into RAM
    SDCard_ReadBMP("7in5_HDb.bmp", 0, 0);

    //Draw red picture
    DEBUG("IMAGE_BWR\n");
    Paint_NewImage(IMAGE_BWR, EPD_7IN5B_HD_WIDTH, EPD_7IN5B_HD_HEIGHT, IMAGE_ROTATE_0, IMAGE_COLOR_POSITIVE);
    Paint_Clear(WHITE);
    SDCard_ReadBMP("7in5_HDr.bmp", 0, 0);

    //4.Refresh the picture in RAM to e-Paper
    EPD_7IN5B_HD_Display();
    DEV_Delay_ms(2000);
#endif
